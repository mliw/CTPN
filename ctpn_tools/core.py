...
from ctpn_tools.text_proposal_connector_oriented import TextProposalConnectorOriented
...
class CTPN:
    ...
    def predict(self, image, output_path):
        ...
        # score > 0.7
        fg = np.where(cls_prod[0, :, 1] > IOU_SELECT)[0]
...
"""
        ...
        if mode == 1:
            select_anchor = utils.clip_box(select_anchor, [h, w])
            for i in select_anchor:
                cv2.rectangle(img,(i[0],i[1]),(i[2],i[3]),color=(255,0,0))
...
The shapes of different layers are as followed
Input_image:(1, 800, 600, 3)
conv5:TensorShape([1, 50, 37, 512])
x:TensorShape([1, 50, 37, 512])
